...
export class ActiveComponent implements OnInit {
  ...
  add() {
    ...
    if (this.values.password !== this.values.passwordTwo) {
      basicAlert(
        TYPE_ALERT.WARNING,
        'Las contraseñas no coinciden y no es valido para activar el usuario. Procura asegurarte que las contraseñas son iguales'
      );
      return;
    }
    // Todo validado vamos enviarlo a la api de graphql
    // servicio => active
    this.usersService
